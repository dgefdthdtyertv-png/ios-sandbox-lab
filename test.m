...
static NSString *logPath = @"/var/mobile/Documents/xpc_log.txt";
...
static void log(NSString *msg) {
    ...
    if (logLabel) {
        dispatch_async(dispatch_get_main_queue(), ^{
            logLabel.text = [logLabel.text stringByAppendingString:line];
        });
    }
}
...
static void test_xpc() {
    log(@"=== XPC Test Start ===");
    xpc_connection_t conn = xpc_connection_create_mach_service("com.apple.containermanagerd", NULL, XPC_CONNECTION_MACH_SERVICE_PRIVILEGED);
    if (!conn) { log(@"xpc connect null"); return; }
    ...
    // 路径探测
    NSArray *paths = @[@"/var/mobile/Containers/Data/Application", @"/var/mobile/Containers/Shared/AppGroup"];
